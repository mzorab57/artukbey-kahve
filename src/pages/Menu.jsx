...
const Menu = () => {
  ...
  const filteredMenuItems = allMenuItems.filter((item) => {
    ...
    if (
      ...
      activeSubcategory !== t("all_coffee")
    ) {
      return (
        item.category === activeCategory && item.type === activeSubcategory
      );
    }
    if (
      activeCategory === t("cold_drink") &&
      activeSubcategory !== t("all_drink")
    ) {
      return (
        item.category === activeCategory && item.type === activeSubcategory
      );
  ...
  return (
    <section
      className="section bg-cover bg-gray-500/10  bg-center w-full min-h-screen py-5"
    ...
    >
      <div className="container mx-auto p-5  max-w-[1100px]">
        ...
        {/* Menu Items */}
        <div
          className="bg-black/40  max-w-[1000px] flex justify-center items-center rounded-lg relative px-5 py-10"
          // style={path === "/" ? { backgroundImage: `url(${menu_logo})`}: {}}
        >
          <img src={menu_logo} alt="j" className="w-96 top-10  absolute" />
          <div className="grid  grid-cols-1 lg:grid-cols-2 gap-x-44  gap-y-14">
            {path === "/" ? (
              allMenuItems
                .slice(0, 6)
                .map((item, index) => (
